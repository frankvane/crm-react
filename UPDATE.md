...
- 在@App.tsx 文件中添加注释内容
## 2025-05-21 07:45:26
- 抽离病患信息加密相关公共方法（importRsaPublicKey、encryptAes、encryptRsaOaep、hybridEncrypt）到 src/pages/goods-order/orders/components/SecretKey.ts，提升代码复用性和可维护性。
- EditPatientModal.tsx、AddPatientModal.tsx 统一从 SecretKey.ts 导入加密方法，移除重复实现。

## 2025-05-20 20:26:07
- 病患详情弹窗（PatientDetailModal）集成病例本地存储：
  1. 新增病例后自动写入 localStorage，数据永久保存。
  2. 页面刷新或切换患者时自动从 localStorage 恢复病例数据，保证病例记录不丢失。
  3. 本地存储按 patientId 区分，互不干扰。
...
1. 组件结构优化
   ...
   - 分离VirtualList和ProductList组件
   - 规范化目录结构
   - 统一样式文件命名为style.module.less
...
1. 优化病患管理页面整体样式：统一圆角、阴影、间距，提升UI一致性和现代感。
2. 优化搜索区域布局，调整输入框和按钮样式，提升可用性和美观度。
3. 优化病患卡片区域，增加卡片hover放大和阴影效果，信息分组更清晰。
4. 优化"添加"按钮样式及悬浮渐变效果。
5. 字体层级和颜色微调，提升可读性。

## 2024-05-19  北京时间  

1. 新增 `useMedicalRecordQuery.ts`，实现 useCreateMedicalRecordMutation，用于创建病例。
2. 在 PatientList.tsx 中：
   - 新增 MedicalRecordModal 组件，实现病例新增弹窗，支持表单录入、提交。
   - 在患者详情弹窗（PatientDetailModal）中，点击"新增"按钮弹出新增病例弹窗，提交后自动刷新病历记录并关闭弹窗。
...
### 新增功能
- 添加了文件头部注释批量处理脚本 `scripts/add-file-headers.js`
  - 支持为多种文件类型添加统一的标准头部注释模板
  - 自动识别已有头部注释的文件，避免重复添加
  - 配置了合理的文件忽略规则，避免处理特定文件类型
  - 通过 `npm run add-headers` 命令快速为项目添加统一文件头

### 优化改进
- 完善了源代码文件管理规范，统一文件头格式
- 添加了示例文件，展示符合规范和不符合规范的情况

## 2023-11-20 更新日志

### 新增功能
- 添加了`require-file-header`自定义ESLint规则，强制要求文件包含标准头部注释
  - 规则会检查文件头部是否包含开发人员和更新时间信息
  - 支持自动修复功能
  - 配置了合理的文件忽略规则，如测试文件、类型声明文件等
