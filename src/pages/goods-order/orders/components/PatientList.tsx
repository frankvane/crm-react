    ...
    >
      <Form
        ...
        onFinish={values => {
          ...
          if (!values.visit_date || typeof values.visit_date.format !== 'function') {
      ...
      >
        <div style={{ marginBottom: 16, background: '#f7f7f7', borderRadius: 6, padding: 12 }}>
          ...
          <div style={{ color: '#444', marginBottom: 4 }}>手机号：{patient?.phone}</div>
  ...
  >
    {record ? (
      <div style={{ lineHeight: 2 }}>
        <div><b>就诊日期：</b>{dayjs(record.visit_date).format('YYYY-MM-DD')}</div>
        <div><b>病况与诊断：</b>{record.diagnosis}</div>
        <div><b>医生建议：</b>{record.treatment}</div>
        <div><b>主治医师：</b>{record.doctor}</div>
        {record.remark && <div><b>备注：</b>{record.remark}</div>}
      </div>
    ) : null}
  </Modal>
);

const PatientDetailModal: React.FC<{ visible: boolean; patientId: number | null; onClose: () => void }> = ({ visible, patientId, onClose }) => {
  const { data, isLoading } = usePatientQuery(patientId || 0, { enabled: !!patientId });
...
const PatientList: React.FC<PatientListProps> = ({ searchParams = {} }) => {
  const { data = { list: [], pagination: { current: 1, pageSize: 20, total: 0 } }, isLoading } = useQuery<PatientListResponse>({
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 8, boxShadow: '0 1px 4px rgba(0,0,0,0.04)', minHeight: 400, padding: 24, width: '100%', marginTop: 16 }}>
      ...
      <PatientDetailModal
        ...
        onClose={() => { setDetailModalVisible(false); setDetailPatientId(null); }}
      />
    </div>
  );
};
