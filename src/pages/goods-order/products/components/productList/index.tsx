...
const ProductList: React.FC<ProductListProps> = ({ categoryId, onShowComments }) => {
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    setImgLoaded({});
    getProducts({ page: 1, pageSize: PAGE_SIZE, category_id: categoryId }).then(res => {
  ...
  const loadMore = useCallback(() => {
    ...
    getProducts({ page: page + 1, pageSize: PAGE_SIZE, category_id: categoryId }).then(res => {
      const list = res?.list || res?.data?.list || [];
  ...
  if (!products.length) return <Empty description="暂无产品" style={{ marginTop: 40 }} />;
  ...
  const Row = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    return (
      <div
        className={index % 2 === 0 ? `${styles.productRow} ${styles.productRowEven}` : styles.productRow}
        style={style}
      >
        <div style={{ position: 'relative', width: 56, height: 56, marginRight: 16 }}>
          <Image
            src={item.image_url}
            alt={item.name}
            ...
            height={56}
            preview={{ mask: '点击预览大图' }}
            placeholder={<Skeleton.Image style={{ width: 56, height: 56 }} active />}
          />
        </div>
        <div className={styles.productInfo} style={{ flex: 3 }}>
          ...
          <div className={styles.productManu}>批准文号：{item.approval_number}</div>
        ...
        <div style={{ flex: 1, color: item.status === 1 ? '#52c41a' : '#d4380d', fontWeight: 500 }}>
          {item.status === 1 ? '上架' : '下架'}
        </div>
        ...
        <div style={{ flex: 'none', marginLeft: 16 }}>
          <button
            style={{ padding: '4px 12px', border: '1px solid #1890ff', borderRadius: 4, background: '#fff', color: '#1890ff', cursor: 'pointer' }}
  ...
  return (
    <>
      ...
      {fetchingMore && <div style={{ textAlign: 'center', padding: 8 }}><Spin size="small" /> 加载更多...</div>}
      {!hasMore && products.length > 0 && (
        <div style={{ textAlign: 'center', color: '#888', padding: 8 }}>没有更多数据</div>
      )}
    </>
  );
};
