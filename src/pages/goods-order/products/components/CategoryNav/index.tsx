...
const CategoryNav: React.FC<CategoryNavProps> = ({ onSelect }) => {
  // 只获取产品分类类型
  const { data: categories = [], isLoading: isCategoryLoading } = useQuery<ICategoryType[]>({
  ...
  const { data: categoryTree = [], isLoading: isCategoryTreeLoading } = useQuery({
    queryKey: ["categories", productCategory?.id],
    queryFn: () => productCategory ? getCategoryTree(productCategory.id) : Promise.resolve([]),
      ...
      <div className={styles.content}>
        <Spin spinning={isCategoryLoading || isCategoryTreeLoading}>
          <Menu
            ...
            defaultOpenKeys={productCategory ? [String(productCategory.id)] : []}
